...
# Control parameters
TRAINING_DAYS = 14     # 2 weeks of rule-based training data (for fair comparison with exploration)
...
def plot_results(training_data, control_data, obs_mean, obs_var, args, folders, mode, timestamp):
    ...
    training_indoor_temps = [d['indoor_temp'] * temp_std + temp_mean for d in training_data]
    training_outdoor_temps = [d['obs'][1] * math.sqrt(obs_var[1]) + obs_mean[1] for d in training_data]
    training_actions = [d['action'] for d in training_data]
    training_power = [d['total_power_demand'] for d in training_data]
    ...
    # Create time indices
    training_time = np.arange(len(training_data)) / (args.timestep * 24)  # Days
    ...
    # Add vertical line to separate training and control periods
    separation_line = TRAINING_DAYS
    for ax in [ax1, ax2, ax3, ax4]:
        ax.axvline(x=separation_line, color='red', linestyle='--', alpha=0.8, linewidth=2)
    
    # Plot temperatures
    ax1.plot(training_time, training_indoor_temps, 'b-', label='Training (Rule-based)', alpha=0.7)
    ax1.plot(control_time, control_indoor_temps, 'r-', label='Control (MPPI)', alpha=0.7)
    ax1.plot(training_time, training_outdoor_temps, 'orange', label='Outdoor Temperature', alpha=0.5)
    ax1.plot(control_time, control_outdoor_temps, 'orange', alpha=0.5)
    ax1.set_ylabel('Temperature (°C)')
    ax1.set_title('HVAC Control Results - Training vs Control')
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    
    # Plot actions
    ax2.plot(training_time, training_actions, 'b-', label='Training Actions', alpha=0.7)
    ...
    # Plot power demand
    ax3.plot(training_time, training_power, 'b-', label='Training Power', alpha=0.7)
    ...
    training_daily_power = []
    control_daily_power = []
    
    for day in range(TRAINING_DAYS):
        day_start = day * timesteps_per_day
        day_end = min((day + 1) * timesteps_per_day, len(training_power))
        if day_end > day_start:
            training_daily_power.append(np.mean(training_power[day_start:day_end]))
    ...
    training_days_x = np.arange(len(training_daily_power))
    control_days_x = np.arange(len(control_daily_power)) + TRAINING_DAYS
    
    ax4.bar(training_days_x, training_daily_power, alpha=0.7, label='Training Daily Avg', color='blue')
    ...
    ax1.text(TRAINING_DAYS/2, ax1.get_ylim()[1]*0.95, 'TRAINING', ha='center', va='top', 
    ...
    training_avg_power = np.mean(training_power)
    ...
    print(f"  Training Period Average: {training_avg_power:.2f} W")
...
def plot_training_data(training_data, obs_mean, obs_var, args, folders, mode, timestamp):
    """
    Create visualization of training data collection with default controller
    ...
    training_hours = [d['hour'] for d in training_data]
    training_indoor_temps = [d['indoor_temp'] * temp_std + temp_mean for d in training_data]
    training_outdoor_temps = [d['obs'][1] * math.sqrt(obs_var[1]) + obs_mean[1] for d in training_data]
    training_actions = [d['action'] for d in training_data]
    training_power = [d['total_power_demand'] for d in training_data]
    
    # Extract continuous actions (heating and cooling setpoints)
    heating_setpoints = [d['continuous_action'][0] for d in training_data]
    cooling_setpoints = [d['continuous_action'][1] for d in training_data]
    
    # Create time indices (in days)
    training_time = np.arange(len(training_data)) / (args.timestep * 24)  # Days
    ...
    total_training_days = len(training_data) / (args.timestep * 24)
    
    # Add comfort zone shading for each day of training period
    comfort_zone_labeled = False
    for day in range(int(total_training_days) + 1):
        # Calculate start and end times for occupied hours each day
        occupied_day_start = day + occupied_start / 24  # 8 AM
        occupied_day_end = day + occupied_end / 24      # 6 PM
    ...
    ax1.plot(training_time, training_indoor_temps, 'b-', label='Indoor Temperature (Default Controller)', alpha=0.7)
    ax1.plot(training_time, training_outdoor_temps, 'orange', label='Outdoor Temperature', alpha=0.7)
    ax1.set_ylabel('Temperature (°C)')
    ax1.set_title('Training Data Collection - Default Rule-Based Controller')
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    
    # Plot discrete actions
    ax2.plot(training_time, training_actions, 'g-', label='Discrete Actions (0-9)', alpha=0.7)
    ...
    ax3.plot(training_time, heating_setpoints, 'r-', label='Heating Setpoint', alpha=0.7)
    ax3.set_ylabel('Heating Setpoint (°C)')
    ax3.legend()
    ax3.grid(True, alpha=0.3)
    
    # Plot cooling setpoints
    ax4.plot(training_time, cooling_setpoints, 'b-', label='Cooling Setpoint', alpha=0.7)
    ax4.set_ylabel('Cooling Setpoint (°C)')
    ax4.legend()
    ax4.grid(True, alpha=0.3)
    
    # Plot power demand
    ax5.plot(training_time, training_power, 'purple', label='Total Power Demand', alpha=0.7)
...
def save_results(training_data, control_data, truth_table, metrics, obs_mean, obs_var, args, folders, mode, timestamp):
    ...
    """
    print("="*60)
    print("Step 8: Saving 2-week baseline results with organized structure")
    print("="*60)
    ...
    training_file = f"{folders['collected_exploration_data']}/2week_baseline_training_data_{mode}_{TRAINING_DAYS}days_{timestamp}.csv"
    training_df.to_csv(training_file, index=False)
    
    # Save control data (data folder)
    control_df = pd.DataFrame(control_data)
    control_file = f"{folders['data']}/2week_baseline_control_data_{mode}_{TRAINING_DAYS}days_{timestamp}.csv"
    ...
    norm_file = f"{folders['data']}/2week_baseline_normalization_params_{mode}_{timestamp}.csv"
    ...
    metrics_file = f"{folders['data']}/2week_baseline_metrics_{mode}_{TRAINING_DAYS}days_{timestamp}.txt"
    with open(metrics_file, 'w') as f:
        f.write(f"MPPI HVAC Control Experiment Results (2-Week Baseline)\n")
        f.write(f"{'='*60}\n")
        f.write(f"Mode: {mode}\n")
        f.write(f"Training days: {TRAINING_DAYS} (2 weeks rule-based)\n")
        f.write(f"Control days: {CONTROL_DAYS} (1 week evaluation)\n")
        f.write(f"Environment: {args.environment}\n")
        f.write(f"Purpose: Baseline comparison for exploration experiment\n")
        f.write(f"\n🔋 ENERGY CONSUMPTION:\n")
...
def run_experiment():
    ...
    folders = create_organized_results_folder('clue', timestamp)
    
    print("="*60)
    print("MPPI HVAC Control Experiment v0.1 (2-Week Baseline)")
    print("="*60)
    print(f"Mode: {'Winter' if args.winter else 'Summer'}")
    print(f"Training days: {TRAINING_DAYS} (2 weeks rule-based for fair comparison)")
    ...
    try:
        ...
        # Step 3: Collect training data with default rule-based controller
        ...
        print("="*60)
        print("2-WEEK BASELINE EXPERIMENT COMPLETED SUCCESSFULLY!")
        print("="*60)
        print(f"Training data collected: {len(training_data)} samples (2 weeks rule-based)")
