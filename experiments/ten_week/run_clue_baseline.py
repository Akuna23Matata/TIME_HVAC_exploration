...
def collect_initial_week_data(env, truth_table, args, week_num=1):
    """
    Collect 1 week of rule-based data (7.1 to 7.7)
    ...
    while step_count < total_week_steps and not (info.get('terminated', False) or info.get('truncated', False)):
        ...
        week_data.append({
            ...
            'phase': 'rule_based_data' if week_num == 1 else 'weekend_exploration',
            'week': week_num,
...
def run_control_exploration_week(env, gp_wrapper, truth_table, all_exploration_data, obs_mean, obs_var, args, week_num):
    ...
    while step_count < total_week_steps:
        ...
        if is_weekday(day_in_week):
            ...
            # WEEKEND: Rule-based Exploration
            action = rule_based_controller(obs, info, args)
...
def plot_weekly_results(all_week_data, weekly_metrics, obs_mean, obs_var, args, folders, mode, timestamp, total_weeks=4):
        ...
        if week_num == 1:
            ...
            if exploration_data:
                ...
                ax1.plot(exploration_time, exploration_indoor_temps, color=week_colors[week_num-1], 
                        label=f'Week {week_num}: Weekend Exploration', alpha=0.6, linewidth=1.5, linestyle='--')
    ...
    ax1.set_title(f'{total_weeks}-Week CLUE HVAC Control Results - Weekly Reset Strategy')
    ax1.legend()
    ...
    # Save figure
    filename = f"{folders['figures']}/{total_weeks}week_clue_results_{mode}_{timestamp}.png"
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    plt.show()
    
    print(f"{total_weeks}-week results plot saved: {filename}")
...
def save_multi_week_results(all_week_data, all_exploration_data, weekly_metrics, truth_table, obs_mean, obs_var, args, folders, mode, timestamp, total_weeks=4):
    ...
    for week_metrics in weekly_metrics:
        ...
        if week == 1:
            phase_desc = "Rule-based Data Collection"
            ...
            phase_desc = "CLUE Control + Weekend Exploration"
            ...
            print(f"      📉 Weekend Exploration: {exploration_violations['violation_rate']*100:.1f}% ({exploration_violations['violation_hours']:.1f} hours)")
        print()
    ...
    exploration_file = f"{folders['collected_exploration_data']}/{total_weeks}week_clue_exploration_data_{mode}_{timestamp}.csv"
    ...
    for week_num, week_data in all_week_data.items():
        ...
        if week_num == 1:
            # Week 1 is all rule-based data
            week1_df = pd.DataFrame(week_data)
            week1_file = f"{folders['data']}/week_{week_num}_rule_based_data_{mode}_{timestamp}.csv"
    ...
    all_data_file = f"{folders['data']}/{total_weeks}week_clue_all_data_{mode}_{timestamp}.csv"
    all_data_df.to_csv(all_data_file, index=False)
    
    # Save weekly metrics (data folder)
    weekly_df = pd.DataFrame(weekly_metrics)
    weekly_file = f"{folders['data']}/{total_weeks}week_clue_weekly_metrics_{mode}_{timestamp}.csv"
    ...
    norm_file = f"{folders['data']}/{total_weeks}week_clue_normalization_params_{mode}_{timestamp}.csv"
    pd.DataFrame([norm_params]).to_csv(norm_file, index=False)
    
    # Save comprehensive metrics (data folder)
    metrics_file = f"{folders['data']}/{total_weeks}week_clue_metrics_{mode}_{timestamp}.txt"
    with open(metrics_file, 'w') as f:
        f.write(f"{total_weeks}-Week CLUE HVAC Control Experiment Results (v0.2)\n")
        f.write(f"{'='*60}\n")
        f.write(f"Mode: {mode}\n")
        f.write(f"Experiment structure (weekly resets to 7.1-7.7):\n")
        f.write(f"  Week 1: 7 days rule-based data collection\n")
        f.write(f"  Weeks 2-{total_weeks}: 5 weekdays CLUE control + 2 weekends exploration + retrain GP\n")
        f.write(f"Environment: {args.environment}\n")
        f.write(f"Total experiment weeks: {total_weeks}\n")
        f.write(f"GP training: Only exploration data (no control data)\n")
        f.write(f"\n📊 WEEKLY PERFORMANCE PROGRESSION:\n")
        f.write(f"{'='*50}\n")
        ...
        for week_metrics in weekly_metrics:
            ...
            if week == 1:
                ...
                f.write(f"    Weekend Exploration: {exploration_violations['violation_rate']*100:.1f}% ({exploration_violations['violation_hours']:.1f} hours)\n")
            f.write(f"\n")
        ...
        for week_num in range(1, total_weeks + 1):
            ...
            if week_num == 1:
                f.write(f"    Week {week_num}: {len(week_data)} rule-based data points\n")
            else:
                control_data = [d for d in week_data if d.get('phase') == 'clue_control']
                exploration_data = [d for d in week_data if d.get('phase') == 'weekend_exploration']
                f.write(f"    Week {week_num}: {len(control_data)} control + {len(exploration_data)} exploration = {len(week_data)} total\n")
    ...
    for week_num in range(1, total_weeks + 1):
        if week_num == 1:
            print(f"    Week {week_num}: rule-based data")
        else:
            print(f"    Week {week_num}: control + exploration data (separate files)")
...
def run_multi_week_experiment():
    ...
    folders = create_organized_results_folder('clue_weekly_reset', timestamp, args.weather)
    
    print("="*60)
    print(f"{TOTAL_EXPERIMENT_WEEKS}-WEEK CLUE HVAC Control Experiment v0.2")
    print("="*60)
    ...
    print(f"  Weeks 2-{TOTAL_EXPERIMENT_WEEKS}: 5 weekdays CLUE control + 2 weekends exploration + retrain GP")
    print(f"Environment: {args.environment}")
    if args.weather:
        print(f"Weather file: {args.weather}")
    ...
    try:
        ...
        print("="*60)
        print(f"{TOTAL_EXPERIMENT_WEEKS}-WEEK CLUE EXPERIMENT COMPLETED SUCCESSFULLY!")
        print("="*60)
        print(f"Week 1 data: {len(week1_data)} samples")
        for week_num in range(2, TOTAL_EXPERIMENT_WEEKS + 1):
            week_data = all_week_data[week_num]
            ...
            print(f"Week {week_num}: {control_count} control + {exploration_count} exploration = {len(week_data)} samples")
        print(f"Total exploration data: {len(all_exploration_data)} samples")
        print("="*60)
